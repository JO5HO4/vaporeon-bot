...
def test_battle_hp_recovers_after_one_hour(tmp_path):
    path = tmp_path / "vaporeon.db"
    now = datetime(2026, 8, 20, tzinfo=timezone.utc)
    apply_splash_damage(9, 30, path, now)
    assert get_battle_hp(9, path, now + timedelta(minutes=59)) == 70
    assert get_battle_hp(9, path, now + timedelta(hours=1)) == 100
    hit = apply_splash_damage(9, 10, path, now + timedelta(hours=1))
